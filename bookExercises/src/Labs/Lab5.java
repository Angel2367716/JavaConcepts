...
//AUTHOR: Marco Angel
//FILENAME: Lab5
//SPECIFICATION: This program producess different designs based on user choice
//FOR: CSE 110 - Lab #5
//TIME SPENT: 50 mins
//-----------------------------------------------------------*/
...
public class Lab5 {
	 ...
	 *
	 * When ptnHeight is not an odd integer, replace it by the closest even integer
	 * below. For example, if numSymbols is 10, use 9 instead.
	 *
	...
	/**
	 * Build a diamond pattern. The parameters are the same as
	 * {@link #buildPyramid(char, char, int)}.
	 *
	 * @param sideSymb   A char to be repeated on both sides
	 * @param midSymb    A char to be repeated in the middle
	...
	private static String buildDiamond(char sideSymb, char midSymb, int numSymbols) {
		...
		for (int numStars = numSymbols; numStars >= 0; numStars = numStars - 2) {
			int numDashed = (totalSymbOneRow - numStars) / 2;
			String row = buildRow(sideSymb, numDashed, midSymb, numStars);
			result += row + "\n";
